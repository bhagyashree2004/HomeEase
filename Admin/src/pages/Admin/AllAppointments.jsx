...
const AllAppointments = () => {
  ...
  const { slotDateFormat,  calculateAge, currency } = useContext(AppContext) //slotDateFormat,
        ...
        {appointments.map((item, index) => (
          <div className='flex flex-wrap justify-between max-sm:gap-2 sm:grid sm:grid-cols-[0.5fr_3fr_1fr_3fr_3fr_1fr_1fr] items-center text-gray-500 py-3 px-6 border-b hover:bg-gray-50' key={index}>
            ...
            <div className='flex items-center gap-2'>
              <img src={item.userData.image} className='w-8 rounded-full' alt="" /> <Link to={`/user-data/${item.userData._id}`} state={{ user: item.userData }}>{item.userData.name}</Link>
            </div>
            {/* <p className='max-sm:hidden'>{item.userData.contact}</p> */}
